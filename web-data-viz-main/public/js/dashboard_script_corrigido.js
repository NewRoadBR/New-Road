...
const MOCK_DATA = {
  ...
  dailyFlow: {
    ...
    // Removed impacto-related placeholders and demo data.
    // This file was trimmed to remove static 'impacto' placeholders per project request.
    document.addEventListener('DOMContentLoaded', () => {
      // Placeholder stub: no-op to avoid page errors.
    });
// ── CHARTS ─────────────────────────────────────────────────
...
async function buildDayChart(rodovia) {
  ...
  const labels =
    dados.map(item => diasSemana[item.dia_semana]);
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          ...
          callbacks: {
            label: ctx =>
              ` ${ctx.parsed.y.toLocaleString('pt-BR')} veículos`
          }
        }
      },
      scales: {
